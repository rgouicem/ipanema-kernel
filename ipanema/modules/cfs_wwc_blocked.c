...
static void ipa_change_proc(struct cfs_ipa_process *proc,
			    struct cfs_ipa_process **dst,
			    enum ipanema_state next_state)
{
	*dst = proc;
	proc->state = next_state;
	proc->rq = NULL;
	change_state(proc->task, next_state, task_cpu(proc->task), NULL);
}

static void ipa_change_queue(struct cfs_ipa_process *proc,
			     ...
			     enum ipanema_state next_state)
{
	...
	proc->state = next_state;
	proc->rq = rq;
	change_state(proc->task, next_state, task_cpu(proc->task), rq);
}


static void ipa_change_queue_and_core(struct cfs_ipa_process *proc,
				      struct ipanema_rq *rq,
				      enum ipanema_state next_state,
				      struct cfs_ipa_core *next_core)
{
	if(proc->state == IPANEMA_RUNNING)
		ipanema_state(task_cpu(proc->task)).current_0 = NULL;
	proc->state = next_state;
	proc->rq = rq;
	change_state(proc->task, next_state, next_core->id, rq);
}
				 ...
				 unsigned int cpu)
{
	...
	ipa_change_proc(p, &ipanema_state(cpu).current_0, IPANEMA_RUNNING);
}
