...
struct state_info {
	...
	struct ipanema_rq blocked; /* public / shared */
};
...
ipanema_batch_get_core_state(struct ipanema_policy *policy,
			     struct core_event *e)
{
			 ...
			 struct batch_ipa_core *thief)
{
	...
	struct ipanema_rq *victim_n_rq, *thief_n_rq, *victim_b_rq, *thief_b_rq;
	unsigned int nr_victim, nr_thief, imbalance;
	unsigned long flags;

	/* Compute number of tasks to steal in order to balance */
	victim_n_rq = &ipanema_state(victim->id).normal;
	victim_b_rq = &ipanema_state(victim->id).blocked;
	thief_n_rq = &ipanema_state(thief->id).normal;
	thief_b_rq = &ipanema_state(thief->id).blocked;

	if (victim_n_rq->nr_tasks == 0)
		return;

	nr_victim = victim_n_rq->nr_tasks + victim_b_rq->nr_tasks;
	nr_thief = thief_n_rq->nr_tasks + thief_b_rq->nr_tasks;

	if (nr_victim <= nr_thief + 1)
		return;

	imbalance = (nr_victim - nr_thief) / 2;

	local_irq_save(flags);

	/* Remove tasks from victim's normal rq */
	detach_tasks(victim_n_rq, imbalance, &stolen_tasks, thief->id);

	/* Add them to my queue */
	attach_tasks(thief_n_rq, &stolen_tasks);

	local_irq_restore(flags);
}

static inline struct batch_ipa_core *
find_idlest_cpu(struct ipanema_policy *policy,
		struct batch_ipa_sched_domain *sd)
{
	unsigned int min_nr = UINT_MAX, nr;
	int cpu;
	struct ipanema_rq *rq;
...
{
	struct batch_ipa_process *tgt = policy_metadata(e->target);
	struct ipanema_rq *rq = &ipanema_state(task_cpu(e->target)).blocked;

	update_runtime(tgt);
	ipa_change_queue(tgt, rq, IPANEMA_BLOCKED);
}
