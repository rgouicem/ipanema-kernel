...
#define  BLOCKED_STATE  (1 << 2)
#define  TERMINATED_STATE  (1 << 3)
#define  READY_TICK_STATE  (1 << 4)
#define  MIGRATING_STATE  (1 << 5)
#define  ACTIVE_CORES_STATE  (1 << 0)
#define  IDLE_CORES_STATE  (1 << 1)
...
static void ipa_change_proc(struct cfs_ipa_process *proc,
			    struct cfs_ipa_process **dst, int state)
{
	*dst = proc;
	proc->state = state;
	proc->rq = NULL;
				  ...
				  struct core_event *e)
{
	...
	struct cfs_ipa_sched_domain *sd;
	ktime_t now = ktime_get();
	u64 interval = ms_to_ktime(cpumask_weight(&policy->allowed_cores));
	int cpu;
	unsigned long flags;
